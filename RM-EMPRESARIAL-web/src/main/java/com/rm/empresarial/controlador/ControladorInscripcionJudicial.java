 ...
 * @author Prueba
 */
@Named(value = "controladorInscripcionJudicial")
@ViewScoped
public class ControladorInscripcionJudicial implements Serializable {
    ...
    @Getter
    @Setter
    private Boolean renderedTabActa = false;
    ...
    public ControladorInscripcionJudicial() {
    ...
    public void llenarDatos() throws ServicioExcepcion, ParseException {
        ...
        deshabilitarBoton = true;
        renderedTabView = false;
        setPreListaRepertorioFecha(inscripcionDao.ListarRepertorioINP_PorFHRPorUsrLog(fecha, dataManagerSesion.getUsuarioLogeado().getUsuId()));
        ...
        for (Repertorio repertorio1 : preListaRepertorioFecha) {
            ...
            if ((repertorio1.getTraNumero().getTraNumero().longValue() == tramit.getTraNumero().longValue()) && !tramit.getTraEstadoRegistro().trim().equals("RZ")) {
                RepertorioUsuario repUsu = repertorioUsuarioServicio.obtenerRepUsu_Por_Rep_Usr_Tipo(repertorio1.getRepNumero(), dataManagerSesion.getUsuarioLogeado().getUsuId(), "INP");
    ...
    public void onRowSelect(SelectEvent event) throws ServicioExcepcion, ParseException {
        ...
        cargarPropiedadDetallePorDefecto();
...
//            }
             */
            //CARGAR PROPIEDADES DE COMPARECIENTES TIPO "S"
            List<Propietario> listaPropietar = new ArrayList<>();
            for (TramiteDetalle tramiteDetalle1 : listaTramiteDetalle) {
                if (tramiteDetalle1.getTtcId().getTtcPropietario().equals("S")) {
                    listaPropietar.clear();
                    listaPropietar = propietarioDao.listarPorPerId(tramiteDetalle1.getPerId().getPerId());
                    for (Propietario propietario : listaPropietar) {
                        if (!propietario.getPrdMatricula().getPrdMatricula().equals("0")) {
                            TramiteValor tramValor = tramiteValorDao.obtenerPor_NumTramite_TipoTramite_Castrato_Predio_Estado(Long.valueOf(numeroTramite), tipoTramite.getTtrId(), propietario.getPrdMatricula().getPrdPredio(), propietario.getPrdMatricula().getPrdCatastro(), "A");
                            if (tramValor != null) {
                                if (!listaPropiedadPorMatricula.contains(propietario)) {
                                    listaPropiedadPorMatricula.add(propietario);
                                }
                            }
                        }
            ...
            propiedadMatricula = listaPropiedadPorMatricula.get(0).getPrdMatricula();
    ...
    public void procesoTerminar() {
        ...
        int aux2 = 0;
        for (Propietario propietario : listaPropiedadPorMatricula) {
            if (propietario.getCantidadPropiedadDetalle() > 0) {
                aux1++;
                listPropDet.clear();
                listPropDet = propiedadDetalleServicio.listar_Por_Matricula_PorEstadoActivo(propietario.getPrdMatricula().getPrdMatricula());
                for (PropiedadDetalle propiedadDetalle : listPropDet) {
                    if (listaPropDetSeleccionadas.contains(propiedadDetalle)) {
                        aux2++;
                    }
                }
            }
        ...
        if (aux2 >= aux1) {
            ...
            JsfUtil.addWarningMessage("Existen propiedades sin seleccionar el Detalle");
        }
    ...
    public void guardarActa() throws ServicioExcepcion, ParseException {
        try {
            ...
            if (actaEncontrada != null) {
                cuerpoActa = actaEncontrada.getActDescripcion15();
                acta();
...
//            numInscripcion = actaServicio.buscarPorAnioYTipoLibroId(anio, tipoTramite.getTplId().getTplId());
            ...
            actaNueva.setActInscripcion(BigInteger.valueOf(0));
            ...
            actaNueva.setActDescripcion15(cuerpoActa);
...
//        actaNueva.setActDescripcion8(txtEditObservaciones);
            if (actaEncontrada != null) {
                if (!fechaDe.after(fechaSentencia) && !fechaDe.after(fechaIngreso) && !fechaSentencia.after(fechaIngreso)) {
                    ...
                    repertorioUsuarioServicio.actualizarEstadoProcesoPor_NumRepertorio_Tipo(repertorio.getRepNumero(), "INP");
                    acta();
                    //razon();
...
//        Date todayWithZeroTime = formatter.parse(formatter.format(today));
        ...
        String numInscripcionActa = "<strong>Nro. de Inscripción: </strong>&nbsp;&nbsp;&nbsp;" + numInscripcion + "<br>";
...
//        cuerpoActa = "";
        ...
        for (TramiteDetalle tramiteDetalle1 : listaTramDetalleCompN) {
            contadorN++;
            if (contadorN < listaTramDetalleCompS.size()) {
                signoPuntuacion = ", ";
            } else {
                signoPuntuacion = ". ";
            }
...
//                + " de la Parroquia de " + nombreParroquia.toUpperCase() + " ________. El presente " + contratoActa
...
//        }
    ...
    public void razon(Tramite tramite) throws ServicioExcepcion {
        ...
        List<Integer> listaNumMat = new ArrayList<Integer>();
        ...
        List<Repertorio> listRepertorio = repertorioDao.listarPorNumTramite(tramite.getTraNumero());
        Repertorio repert = new Repertorio();
        for (Repertorio repertorio1 : listRepertorio) {
            repert = repertorioDao.encontrarRepertorioPorId(repertorio1.getRepNumero().toString());
        }
        ...
        String fechaRazon = fechasUtil.convertirFechaA_diaSemana_ddMMAAAA_hhmmss(rep.getRepFHR());
    ...
    public void sobreEscribirActa() {
        try {
            ...
            actaActualizar.setActDescripcion15(cuerpoActa);
        ...
        try {
            ...
            for (Repertorio repertorio1 : listRepertorio) {
                ...
                numRepUsuActualizados = repertorioUsuarioServicio.actualizarEstadoRepUsuPorRepPorUsrPorTipo(repertorio1.getRepNumero(),
                        dataManagerSesion.getUsuarioLogeado().getUsuId(), "INP");
...
//                prop = propieadadDao.buscarPropiedadPor_predio_o_catastro(tramValor.getTraNumPredio(), tramValor.getTrvNumCatastro());
//                if (prop != null && !prop.getPrdMatricula().equals("0")) {
//                    listaProp.add(prop);
//                }
//
//            }
            List<Propiedad> listaProp = new ArrayList<>();
            for (Propietario propietario : listaPropiedadPorMatricula) {
                listaProp.add(propietario.getPrdMatricula());
            }
            ...
            for (Propiedad propiedad : listaProp) {
                ...
                if (listaGravamen.isEmpty()) {
                    if (!propiedad.getPrdMatricula().equals("0")) {
...
//                    Gravamen gravamenBuscar = gravamenDao.buscarTopPorMatriculaYRepertorioYEstado(Integer.valueOf(numeroRepertorio), propiedad.getPrdMatricula(), "A");
//                    //generarGravamenDetalle();
//                    for (GravamenDetalle gravamenDetalle : listaGravamenDetalle) {
//                        gravamenDetalle.setGraId(gravamenBuscar);
//                        gravamenDetalleDao.create(gravamenDetalle);
//                    }
                }

            }
            //crea gravamen detalle           

            generarGravamenDetalle();

            /**
             * ******************* crea lista de comparecientes tipo S y N
             * ************************
             */
            Propiedad propiedadMatriculaCero = new Propiedad();
            propiedadMatriculaCero = propieadadDao.encontrarPropiedadPorId("0");
            List<TramiteDetalle> listTramDet = new ArrayList<>();
            List<TramiteDetalle> listaTramDetCompN = new ArrayList<>();
            List<TramiteDetalle> listaTramDetCompS = new ArrayList<>();
            tramiteDetalle = tramiteDetalleDao.buscarPorNumTramiteYDescripcion(Long.parseLong(numeroTramite), descripcionContrato);
            tipoTramite = tipoTramiteDao.buscarPorID(tramiteDetalle.getTdtTtrId());
            Repertorio repert = new Repertorio();
            repert = repertorioDao.encontrarRepertorioPorId(numeroRepertorio);
            listTramDet = tramiteDetalleDao.listarPorNumTramitePorIdTipoTramitePorEstadoPorRepertorio(
                    Long.valueOf(numeroTramite), repert.getRepTtrId(), Long.valueOf(numeroRepertorio));

            listaTipoCompareciente = tipoTramiteComparecienteDao.listarTipoComparecientePorTipoTramite(Long.valueOf(repert.getRepTtrId()));

            for (TramiteDetalle tramDetalle : listTramDet) {
                TipoTramiteCompareciente tipoTramCompareciente = new TipoTramiteCompareciente();
                tipoTramCompareciente = tipoTramiteComparecienteDao.buscarPorId(tramDetalle.getTtcId().getTtcId());
                if (tipoTramCompareciente.getTtcPropietario().equals("N")) {
                    listaTramDetCompN.add(tramDetalle);
                } else if (tipoTramCompareciente.getTtcPropietario().equals("S")) {
                    listaTramDetCompS.add(tramDetalle);

                }
            }

            /**
             * ****** CREA GRAVAMEN (CON MATRICULA CERO) Y GRAVAMEN DETALLE DEL
             * COMPARECIENTE QUE NO TIENE PROPIEDADES ********************
             */
            for (TramiteDetalle tramiteDetalle1 : listaTramDetCompS) {
                List<Propietario> listaPropietar = new ArrayList<>();
                listaPropietar = propietarioDao.listarPor_Id_Persona(tramiteDetalle1.getPerId().getPerId());

                if (listaPropietar.isEmpty()) {

                    int existeGravemen = gravamenDao.buscarPorRepertorio_IdPersona_Estado(Long.valueOf(
                            numeroRepertorio), tramiteDetalle1.getPerId().getPerId(), "A");
                    ...
                    for (Propietario propietario : listaPropietar) {
                        ...
                        if (existePropiedad == false) {
                            ...
                            int existeGravemne = gravamenDao.buscarPorRepertorio_IdPersona_Estado(Long.valueOf(
                                    numeroRepertorio), tramiteDetalle1.getPerId().getPerId(), "A");
    ...
    public void cargarPropiedadDetallePorDefecto() throws ServicioExcepcion {
        listaPropDetSeleccionadas.clear();
        listaPropiedadDetalle.clear();
        Repertorio repert = new Repertorio();
        repert = repertorioDao.encontrarRepertorioPorId(numeroRepertorio);
        ...
        for (Propietario propietario : listaPropiedadPorMatricula) {
            ...
            listaPropiedadDetalle = propiedadDetalleServicio.listar_Por_Matricula_PorEstadoActivo(propietario.getPrdMatricula().getPrdMatricula());
            for (PropiedadDetalle propiedadDetalle : listaPropiedadDetalle) {
                Persona persona = personaServicio.buscarPersonaPorId(propiedadDetalle.getPdtPerId().longValue());
                propiedadDetalle.setPersona(persona);
                Long perId = propiedadDetalle.getPdtPerId().longValue();
                for (TramiteDetalle tramiteDetalle1 : listaTramiteDetalle) {
                    if (tramiteDetalle1.getTtcId().getTtcPropietario().equals("S")) {
                        if (tramiteDetalle1.getPerId().getPerId().longValue() == propiedadDetalle.getPdtPerId().longValue()) {
                            if (!listaPropDetSeleccionadas.contains(propiedadDetalle)) {
                                listaPropDetSeleccionadas.add(propiedadDetalle);
                            }
                        }
    ...
    public void generarGravamenDetalle() throws ServicioExcepcion {
        listaGravamenDetalle.clear();
        for (PropiedadDetalle listaPropDetSeleccionada : listaPropDetSeleccionadas) {
            if (listaPropDetSeleccionada.getPrdMatricula().getPrdMatricula() != null) {
                Propiedad prop = new Propiedad();
                prop = listaPropDetSeleccionada.getPrdMatricula();

                Gravamen gravamenBuscar = gravamenDao.buscarTopPorMatriculaYRepertorioYEstado(Integer.valueOf(numeroRepertorio), prop.getPrdMatricula(), "A");
                if (gravamenBuscar != null) {
                    List<GravamenDetalle> listGravDet = new ArrayList<>();
                    listGravDet = gravamenDetalleDao.listarPorGravamenId_PorIdPersona_PorEstado(listaPropDetSeleccionada.getPersona().getPerId(), gravamenBuscar.getGraId(), "A");
                    if (listGravDet.isEmpty()) {
                        GravamenDetalle gravamenDetalle = new GravamenDetalle();
                        gravamenDetalle.setPerId(listaPropDetSeleccionada.getPersona());
                        gravamenDetalle.setGvdEstado("A");
                        gravamenDetalle.setGvdPorcentaje(listaPropDetSeleccionada.getPdtValor());
                        gravamenDetalle.setGvdFHR(Calendar.getInstance().getTime());
                        gravamenDetalle.setGvdUser(dataManagerSesion.getUsuarioLogeado().getUsuLogin());
                        gravamenDetalle.setGraId(gravamenBuscar);
                        gravamenDetalleDao.create(gravamenDetalle);
                        //listaGravamenDetalle.add(gravamenDetalle);
                    }
    ...
    public void verificarEstadoProceso(Tramite tramiteTerm) {
        ...
        for (Repertorio repertor : listaRepertorioFecha) {
            if (repertor.getTraNumero().getTraNumero().longValue() == tramiteTerm.getTraNumero().longValue()) {
                RepertorioUsuario repUsu = repertorioUsuarioServicio.obtenerRepUsu_Por_Rep_Usr_Tipo(repertor.getRepNumero(), dataManagerSesion.getUsuarioLogeado().getUsuId(), "INP");
