...
import com.rm.empresarial.dao.IncidenciaDao;
 ...
 * @author MP
 */
@Named(value = "controladorIncidenciaActa")
@ViewScoped
public class ControladorIncidenciaActa implements Serializable {
...
//    private List<TramiteDetalle> listaTramDetalleCompN = new ArrayList<>();
    ...
    @Getter
    @Setter
    private Incidencia incidenciaActa;
    @EJB
    private IncidenciaDao incidenciaDao;

    public ControladorIncidenciaActa() {
        antecedentes = "";
        compareciente = "";
        cuantias = "";
        gravamenes = "";
...
//        actaA = new Acta();
    ...
    public void onRowSelect(Incidencia incidencia) throws ServicioExcepcion, ParseException {

        incidenciaActa = new Incidencia();
        incidenciaActa = incidencia;
        limpiarCamposTxtEdit();
        cargarDatosCantonNotaria();

        Repertorio repert = new Repertorio();

        numRepertorio = incidencia.getIncRepNumero().toString();
        repert = repertorioDao.encontrarRepertorioPorId(numRepertorio);
        numTramite = repert.getTraNumero().getTraNumero().toString();
        contratoDescripcion = repert.getRepTtrDescripcion();
        ...
        if (esActaNueva) {
            ...
            txtEditCopiaTextual = actaEncontrada.getActDescripcion9();
...
//                    txtEditObjetoContrato = adjudicacion.txtObjetoContrato();
                    ...
                    break;
                default:
                    JsfUtil.addErrorMessage("Tipo de Trámite no parametrizado");
...
//                    context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Aviso", mensaje));
            ...
            JsfUtil.addErrorMessage("Código no parametrizado para el Tipo de Trámite seleccionado");
...
//            pdfTempUtil.agregarImagenEspacioEnBlanco();
    ...
    public void generarPdfRazon() {
        try {
            pdfTempUtil.crearPDFconTextoHTML("razon.pdf", razonInscripcion, "RAZ");
            pdfTempUtil.generarLinksAccesoAlPdf();
    ...
    public void generarActaParaHTML() throws ServicioExcepcion, ParseException {
        ...
        if (tramiteDetalle != null && !tramiteDetalle.getTdtTtrDescripcion().trim().equals("TESTAMENTO")) {
            ...
            setActaHTML(cabeceraHTML + "<br></br>" + getTestamento());
            actaHTML = actaHTML.replaceAll("null", "");
            deshabilitarBotonGuardar = false;
        }
...
//            }
    ...
    public void preGuardarActa() throws ServicioExcepcion, ParseException {
        ...
        TipoMarginacion tipoMrgOtros = new TipoMarginacion();
        tipoMrgOtros = servicioTipoMarginacion.find(new TipoMarginacion(), new Long(13));
        ...
        nuevaMarginacion.setMrgAlt1("MARGINACIÓN POR MODIFICACIÓN DE ACTA");
        //campos no requeridos
        nuevaMarginacion.setTmcId(tipoMrgOtros);
        nuevaMarginacion.setActNumero(actaEncontrada);
        nuevaMarginacion.setMrgAlt2(numRepertorio);
        nuevaMarginacion.setMrgAlt3("");
        nuevaMarginacion.setMrgFch(repertorio.getRepFHR());
        servicioMarginacion.create(nuevaMarginacion);
        
        limpiarCamposTxtEdit();      
        controladorIncidenciaUsuario.setRendPnlActa(false);
    ...
    public void guardarActa() throws ServicioExcepcion, ParseException {
        ...
        if (esActaNueva) {
            ...
            incidenciaActa.setIncEstado("T");
            incidenciaDao.edit(incidenciaActa);
            ...
            for (TramiteDetalle tramDetalle : listaTramiteDetalle) {
                ...
                if (tipoTramComparec.getTtcPropietario().equals("N")) {
                    for (RepertorioPropiedad repPropiedad : listaRepertorioPropiedad) {
                        ...
                        Boolean existePropietario = propietarioDao.existePropietarioPor_Persona_Matricula_Repertorio(tramDetalle.getPerId().getPerId(), repPropiedad.getPrdMatricula().getPrdMatricula(), new Long(numRepertorio));
...
//                System.out.print("Secuencia-- " + auxSecuencia);
                        ...
                        actaGuardar.setActDescripcion9(txtEditCopiaTextual);
                        //llenarDatos();
                        //limpiarCamposTxtEdit();
    ...
    public void listarComparecientes() throws ServicioExcepcion, ParseException {
        try {
            numRepertorio = incidenciaActa.getIncRepNumero().toString();
            Repertorio repert = new Repertorio();
            repert = repertorioDao.encontrarRepertorioPorId(numRepertorio);
    ...
    public void buscarActa() {
        try {
            ...
            if (actaEncontrada != null) {
                esActaNueva = false;
                JsfUtil.addSuccessMessage("Acta Encontrada");
            } else {
                esActaNueva = true;
            }
...
//            }
        ...
        certificadoGenerado += ""
                + "<p align='center'><strong>REGISTRO DE LA PROPIEDAD DEL CANTÓN QUITO </strong></p>"
        ...
        try {
            ...
            for (Repertorio repertor : listaRepertorioFecha) {
                ...
                if (repertor.getTraNumero().getTraNumero().longValue() == tramite.getTraNumero().longValue()) {
                    ...
                    tipoLibro = tipoLibroDao.encontrarTipoLibroPorId(tipoTramite.getTplId().getTplId().toString());
                    linea2 = linea2 + "REGISTRO DE " + repertor.getRepTtrDescripcion()
                            + ", " + "tomo " + tipoLibro.getTplTomo() + ", repertorio(s) - " + repertor.getRepNumero() + ".<br><br>";
...
//                propiedad = propiedadDao.encontrarPropiedadPorId(numMat);
            ...
            String textoFinal = "Los número de matrícula le servirán para cualquier trámite posterior.<br></br><br></br>"
                    + "Responsables.- <br></br>";
            String asesor = "Asesor.- " + "<br></br>";
            String revisor = "Revisor.- " + "<br></br><br></br><br></br>";
            String numRazon = "RR-";
...
//                System.out.print("Secuencia-- " + auxSecuencia);
    ...
    public void guardar() throws ServicioExcepcion, ParseException {
        ...
        for (Repertorio repertorio1 : listaRep) {
            repProp.clear();
            repProp = repertorioPropiedadDao.buscarPorNumRepertorio(repertorio1.getRepNumero());
            repPropMat.addAll(repProp);
        }
        //obtener propiedades que faltan por generar certificado
        for (RepertorioPropiedad repPropie : repPropMat) {
            for (CertificadoDatos certifDato : listaCertificadoDatos) {
                if (certifDato.getPropiedad().getPrdMatricula().equals(repPropie.getPrdMatricula().getPrdMatricula())) {

                }
            }

        }
        if (listaCertificadoDatos.size() == repPropMat.size()) {
            ...
            TipoTramite tipoTamRepActual = tipoTramiteDao.buscarID(repertorio.getRepTtrId());
...
class CertificadoDatosIncidencia {
    ...
    public CertificadoDatosIncidencia(Propiedad propiedad, Date fechaIngreso, String antecedentes, String certficadoPDF, String descripcion, String nombrePropiedatarios, String gravamenes, String certNumero, Short secuencial) {
