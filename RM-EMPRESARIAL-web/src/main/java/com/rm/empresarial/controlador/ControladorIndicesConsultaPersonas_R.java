 ...
 * @author DJimenez
 */
@Named(value = "controladorIndicesConsultaPersonas_R")
@ViewScoped
public class ControladorIndicesConsultaPersonas_R implements Serializable {
    ...
    @EJB
    private CercontratoServicio servicioCercontrato;
    @EJB
    private DinardapServicio servicioDinardap;
    @EJB
    private IndicePropServicio servicioIndiceProp;
    @EJB
    private MarginacionRServicio servicioMarginacionR;
    ...
    @Getter
    @Setter
    private Cercontrato cerContrato;
    @Getter
    @Setter
    private Dinardap dinarDap;
    @Getter
    @Setter
    private MarginacionR marginacionR;
    @Getter
    @Setter
    private IndiceProp indiceProp;
    ...
    @Getter
    @Setter
    private List<Cercontrato> listaCerContrato;
    @Getter
    @Setter
    private List<Dinardap> listaDinardap;
    @Getter
    @Setter
    private List<MarginacionR> listaMarginacionR;
    @Getter
    @Setter
    private List<IndiceProp> listaIndiceProp;
    ...
    public ControladorIndicesConsultaPersonas_R() {
        ...
        dinarDap = new Dinardap();
        marginacionR = new MarginacionR();
        indiceProp = new IndiceProp();
        ...
        listaCerContrato = new ArrayList<>();
        listaDinardap = new ArrayList<>();
        listaMarginacionR = new ArrayList<>();
        listaIndiceProp = new ArrayList<>();
    ...
    public void procesarConsulta() throws ServicioExcepcion {
        ...
        listaMarginacionR = new ArrayList<>();
        listaIndiceProp = new ArrayList<>();
        if (identificacion.isEmpty() && apellidoPaterno.isEmpty() && apellidoMaterno.isEmpty() && nombre.isEmpty()
                && indfecha == null && indiceProp.getIndRepertorio() == null && indiceProp.getIndInscripcion() == null
                && indiceProp.getIndCrva() == null && cerContrato.getCerCrva() == null && marginacionR.getMarLibro() == null && marginacionR.getMarAnio() == null && marginacionR.getMarInscripcion() == null) {
            JsfUtil.addErrorMessage("Ingrese almenos un dato para relizar la búsqueda");
        } else {
            if (!identificacion.isEmpty() || !apellidoPaterno.isEmpty() || !apellidoMaterno.isEmpty() || !nombre.isEmpty()) {
                if (!apellidoPaterno.isEmpty() || !apellidoMaterno.isEmpty() || !nombre.isEmpty()) {
                    String nombreAux = nombre;
                    String apellidoPaternoAux = apellidoPaterno;
                    String apellidoMaternoAux = apellidoMaterno;
                    String nombreC = "%" + apellidoPaternoAux + "%" + apellidoMaternoAux + "%" + nombreAux + "%";
                    setListaIndiceProp(servicioIndiceProp.listarIndiceProp_Like_Nombre(nombreC));
                }
...
//            setListaPersonas(servicioPersona.listarPersonasPorLike(identificacion, apellidoPaterno.toUpperCase(), apellidoMaterno.toUpperCase(), nombre.toUpperCase()));
            ...
            } else if (indfecha != null && indiceProp.getIndRepertorio() != null && indiceProp.getIndInscripcion() != null) {
                setListaIndiceProp(servicioIndiceProp.listarIndiceProp_Por_Fecha_Repertorio_Inscrpcion(indfecha, indiceProp.getIndRepertorio(), indiceProp.getIndInscripcion()));
                indiceProp = new IndiceProp();
                indfecha = null;
            } else if (indiceProp.getIndCrva() != null) {
                setListaIndiceProp(servicioIndiceProp.listarIndiceProp_Por_Crva(indiceProp.getIndCrva()));
                indiceProp = new IndiceProp();
            } else if (cerContrato.getCerCrva() != null) {
                setListaCerContrato(servicioCercontrato.listarCerContrato_Por_Crva(cerContrato.getCerCrva()));
                setListaDinardap(servicioDinardap.listarDinarDap_Por_Crva(cerContrato.getCerCrva()));
                cerContrato = new Cercontrato();
            } else if (marginacionR.getMarLibro() != null && marginacionR.getMarAnio() != null && marginacionR.getMarInscripcion() != null) {
                setListaMarginacionR(servicioMarginacionR.listarMarginacionR_Por_libro_Anio_Inscripcion(marginacionR.getMarLibro().intValue(), marginacionR.getMarAnio().intValue(), marginacionR.getMarInscripcion()));
                marginacionR = new MarginacionR();
            } else {
                JsfUtil.addErrorMessage("Nose encontró resultados de la búsqueda con los parámetros ingresados");
            }
