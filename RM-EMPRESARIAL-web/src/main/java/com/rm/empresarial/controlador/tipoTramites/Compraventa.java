...
public class Compraventa implements Serializable {
    ...
    public Propiedad verificarPropiedad(Propiedad propiedad) {
        ...
        propDet = propiedadDetalleDao.obtener_Por_PdtPrdMatricula_EstadoActivo(propiedad.getPrdMatricula(), "A");
        if (propDet != null) {
    ...
    public String txtCompareciente(String numRepertorio, String numTramite) throws ServicioExcepcion {
        ...
        for (TramiteDetalle listaTxtCompareciente : listaTramiteDetalle) {
            ...
            if (listaTxtCompareciente.getPerId().getPerEstadoCivil() == null) {
                comparecietes = comparecietes + listaTxtCompareciente.getPerId().getPerNombre().trim()
...
//        listaRepertorioPropiedad = repertorioPropiedadDao.buscarPorNumRepertorio(Long.valueOf(numRepertorio));
        ...
        for (RepertorioPropiedad repertorioPropiedad : listRepPropAux) {
            for (TramiteDetalle tramDet : listaTramiteDetalle) {
                if (tramDet.getTtcId().getTtcPropietario().equals("N")) {
...
//        listaRepertorioPropiedad = repertorioPropiedadDao.buscarPorNumRepertorio(Long.valueOf(numRepertorio));        
        ...
        for (Propiedad propied : listPropVerificadas) {
            listaLindero = linderoDao.listarPorNumMatricula(propied.getPrdMatricula());
            String txtLinderos = "";
            for (Lindero lindero : listaLindero) {
                txtLinderos = txtLinderos + lindero.getLdrNombre().trim() + ", ";
            }

            linderosActa = linderosActa + "Matrícula: " + propied.getPrdMatricula()
...
//        String sql2 = "SELECT PrdMatricula FROM RepertorioPropiedad where RepNumero = " + numRepertorio + " ORDER BY PrdMatricula";
//        numMatricula = (repertorioPropiedadDao.ObtenerPropiedadNumMatricula(sql2));
